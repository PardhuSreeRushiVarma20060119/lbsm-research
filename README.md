![arts-lbsm](arts/lbsm-01.png)

### Latent Behavioral Structure in Low-Dimensional Statistical Manifolds (LBSM)

LBSM is an experimental statistical machine learning research investigating whether adaptive behavioral telemetry generated by autonomous systems exhibits structured low-dimensional latent organization despite high-dimensional observable dynamics.

## Environment Notes

- SageMath is required for this project.
- Install SageMath via the Nix development shell (`flake.nix`) or system-level SageMath installation.
- `requirements.txt` is for PyPI-resolvable Python packages only and intentionally does not include `sage`.

The project explores:
- latent behavioral structure discovery
- behavioral manifold formation
- temporal behavioral evolution
- reinforcement-guided adaptive dynamics
- probabilistic behavioral inference
- statistical observability of adaptive systems

---

## Core Hypothesis

Adaptive behavioral systems generate observable telemetry that occupies structured low-dimensional statistical manifolds rather than arbitrary high-dimensional space.

---

## Research Direction

LBSM studies:
- whether behavioral regimes naturally cluster in latent space
- whether reinforcement-guided adaptation induces geometric structure
- whether behavioral drift manifests as manifold displacement
- whether temporal behavioral evolution admits compact statistical representations

---

## Status

> Current experimental artifacts, telemetry outputs, and generated research assets.

#### 📓 Notebooks

- Notebook-01 : 
<p>
  <a href="notebooks/01_telemetry_generation.ipynb">
    <img src="https://img.shields.io/badge/Open-Notebook-blue?style=for-the-badge&logo=jupyter">
  </a>
</p>

- Notebook-02 :
<p>
  <a href="notebooks/02_manifold_learning.ipynb">
    <img src="https://img.shields.io/badge/Open-Notebook-blue?style=for-the-badge&logo=jupyter">
  </a>
</p>

- Notebook-03 :
<p>
  <a href="notebooks/03_hmm_inference.ipynb">
    <img src="https://img.shields.io/badge/Open-Notebook-blue?style=for-the-badge&logo=jupyter">
  </a>
</p>

#### 🗂️ Datasets

- t2000 dataset (nb01) :
<p>
  <a href="data/processed/nb01/telemetry_n20_t2000.csv">
    <img src="https://img.shields.io/badge/Open-Dataset-green?style=for-the-badge&logo=databricks">
  </a>
</p>

- trajectory & transistion coords dataset (nb02) :
<p>
  <a href="data/processed/nb02/trajectory_stats.csv">
    <img src="https://img.shields.io/badge/Open-Dataset-green?style=for-the-badge&logo=databricks">
  </a>
  <a href="data/processed/nb02/transistion_coords.csv">
    <img src="https://img.shields.io/badge/Open-Dataset-green?style=for-the-badge&logo=databricks">
  </a>
</p>

- hmm datasets: metrics and accuracy (nb03) :
<p>
<a href="data/processed/nb03/hmm_agent_metrics.csv">
  <img src="https://img.shields.io/badge/Open-Dataset-green?style=for-the-badge&logo=databricks">
</a>
<a href="data/processed/nb03/hmm_regime_accuracy.csv">
  <img src="https://img.shields.io/badge/Open-Dataset-green?style=for-the-badge&logo=databricks">
</a>
</p>

---
